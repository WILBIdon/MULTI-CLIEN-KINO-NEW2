...
try {
    ...
    // --- LOGICA DE ENLAZADO (Algoritmo Usuario) ---

    // 1. Asegurar índice único (Recomendación Usuario)
    try {
        $db->exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_original_path_lower ON documentos(original_path)");
    } catch (Exception $e) { /* Ignorar si falla */
    }

    // 2. Pre-Check: Poblar original_path si existe la columna 'path' (y original_path esta vacio)
    // Esto es útil si la tabla tiene una columna 'path' legacy
    try {
        $cols = $db->query("PRAGMA table_info(documentos)")->fetchAll(PDO::FETCH_COLUMN, 1);
        if (in_array('path', $cols)) {
            $db->exec("UPDATE documentos SET original_path = path WHERE (original_path IS NULL OR original_path = '') AND path IS NOT NULL");
        }
    } catch (Exception $e) {
    }
    ...
    if ($zip->open($zipFile['tmp_name']) === TRUE) {
        ...
        // Prepara consultas reutilizables (Adaptadas a SQLite)

        // Estrategia 0: Match exacto por `original_path`
        $stmtPath = $db->prepare(
            "UPDATE documentos SET ruta_archivo = ?, original_path = ?
             WHERE LOWER(original_path) = LOWER(?) OR LOWER(ruta_archivo) = LOWER(?)"
        );

        // Estrategia 1: Match Exacto por Numero
        $stmtExact = $db->prepare(
            "UPDATE documentos SET ruta_archivo = ?, original_path = ?
             WHERE TRIM(LOWER(numero)) = TRIM(LOWER(?))"
        );

        // Estrategia 2: Prefijos comunes
        // LOWER(?) LIKE LOWER(numero || sep || '%')
        $stmtPrefix = $db->prepare(
            "UPDATE documentos SET ruta_archivo = ?, original_path = ?
             WHERE LOWER(?) LIKE LOWER(numero || ? || '%')"
        );

        // Estrategia 3: Tokenización
        $stmtTok = $db->prepare(
            "UPDATE documentos SET ruta_archivo = ?, original_path = ?
             WHERE LOWER(numero) LIKE LOWER(?) AND (ruta_archivo IS NULL OR ruta_archivo = '' OR ruta_archivo = 'pending' OR ruta_archivo = 'importado')"
        );

        // Consultas para Autocuración
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            $numero = $baseName; // Asumimos numero = filename base inicialmente

            // Estrategia 0: Match exacto path
            $stmtPath->execute([$relativePath, $filename, $filename, $filename]);
            if ($stmtPath->rowCount() > 0) {
                $updatedDocs++;
                logMsg("✅ Vinculado (Path): $filename", "success");
                continue;
            }

            // Estrategia 1: Match Exacto nombre
            $stmtExact->execute([$relativePath, $filename, $baseName]);
            ...
            // Estrategia 2: Prefijos
            $separators = ['_', '-', ' '];
            $matched = false;
            foreach ($separators as $sep) {
                // Check: LOWER(filename) LIKE LOWER(numero || sep || '%')
                $stmtPrefix->execute([$relativePath, $filename, $filename, $sep]);
                if ($stmtPrefix->rowCount() > 0) {
                    $updatedDocs++;
                    logMsg("✅ Vinculado (Prefijo '$sep'): $baseName", "success");
                    $matched = true;
                    break;
                }
            }
            ...
            foreach ($tokens as $token) {
                if (strlen($token) < 4)
                    continue; // Min 4 chars

                $stmtTok->execute([$relativePath, $filename, '%' . $token . '%']);
                if ($stmtTok->rowCount() > 0) {
                    $updatedDocs++;
                    logMsg("✅ Vinculado (Token '$token'): $baseName", "success");
                    $foundToken = true;
                    break;
                }
            }
        ...
        // Fase de Autocuración y Creación
        foreach ($unlinkedFiles as $fileName) {
            // Comprobación exacta por original_path
            $stmtCheckPath->execute([$fileName]);
            if ($stmtCheckPath->fetchColumn()) {
                continue; // Ya existe
            }

            $numero = pathinfo($fileName, PATHINFO_FILENAME);
            $relativePath = 'sql_import/' . $fileName;
            $fecha = date('Y-m-d');

            // Autocuración: Check por Número
            $stmtCheckNumber->execute([$numero]);
            $docId = $stmtCheckNumber->fetchColumn();

            if ($docId) {
                // Recuperar Huérfano
                $stmtUpdateLink->execute([$relativePath, $fileName, $docId]);
                $updatedDocs++;
            ...
            // Crear Nuevo
            try {
                $stmtCreate->execute(['generado_auto', $numero, $fecha, 'Importación Auto', 'procesado', $relativePath, $fileName]);
                $createdDocs++;
                logMsg("✨ Creado Autom.: $numero", "success");
            } catch (Exception $e) {
